...
def MoveQueens(current_board, counter, sideway_counter):
    ...
    if parent_h != 0:
        ...
        if value <= parent_h:
            ...
            if value < parent_h:
                print("Move")
                print(child_board)
                return MoveQueens(child_board, counter + 1, 0)
            elif value == parent_h:
                if sideway_counter >= 100:
                    ...
                    print("Move")
                    print(child_board)
                    return MoveQueens(child_board, counter + 1, sideway_counter)
        else:
            print("Fail")
            return counter, "F"
    else:
...
def GenerateBoard():
    ...
    while counter_avg > 0:
        ...
        print("Initial Board")
        print(current_board)
        steps_count, goal_test = MoveQueens(current_board, 1, 0)
        if goal_test == "F":
    ...
    print("Rates: ")
    print("Failure rate: ", round((f_counter / ExecutionCycle) * 100, 2), " Success rate: ",
          round((g_counter / ExecutionCycle) * 100, 2),
